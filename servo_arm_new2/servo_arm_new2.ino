...
class Servo_Motor
{
  ...
  int update_pos(int curr_pos,int final_pos,int dir);     //curr_pos  will contain the pos of current passing object
}m[12];
...
/*  case 'C':
  ...
  case '1':
  m[0].move_Motor(60,m[0].pos,0,2,0);
  break;
...

{
 ...
 Serial.println(pos);
 m[motor_num].dir=dir;
  if(m[motor_num].dir==1)                //rotate clockwise
  {
    ...
    if(m[motor_num].pos<179)
      {
        int a=update_pos(m[motor_num].pos,deg,m[motor_num].dir);
    ...
    if(m[motor_num].dir==0)
    {
    Serial.println("Executing cond ");
    Serial.print("Val of pos  ");
    Serial.println(m[motor_num].pos);
    if(m[motor_num].pos>0)
      {
        int a=update_pos(m[motor_num].pos,deg,dir);
        Serial.print("Val of a      :");
        Serial.println(a);
        for(i=m[motor_num].pos;   i>a;  i--){
          s[servo_obj].write(i);
  ...
  int Servo_Motor::update_pos(int curr_pos,int final_pos,int d)//d for direction
  {
    Serial.print("Current pos  :");
    Serial.println(curr_pos);
    //Serial.print("final_pos");
    Serial.print("Val of d");
    Serial.println(d);
